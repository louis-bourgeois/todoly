...
const TRANSITION_DURATION = 150; // en millisecondes
const ICON_WIDTH = 25; // Vous pouvez modifier cette valeur
...
const Wrapper = React.memo(
  ({ children, className, style, onClick, disabled, ...props }) => (
    <button
...
const IconSvg = React.memo(({ className = "" }) => (
  <svg
...
const CTA = ({
  ...
  useEffect(() => {
    if (wrapperRef.current && contentRef.current) {
      wrapperRef.current.style.width = `${contentRef.current.offsetWidth}px`;
    }
  }, [displayIcons, title]);
    ...
    >
      ...
      >
        {type === "primary" && showIcons && (
          <div style={iconStyle}>
            <IconSvg fill={isHovered ? "white" : "black"} />
          </div>
        )}
        <span>{title}</span>
        {type === "primary" && showIcons && (
