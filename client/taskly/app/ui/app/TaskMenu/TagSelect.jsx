...
export default function TagSelect({
  ...
  const handleAddTag = (name = "", id = undefined) => {
    if (taskTags.some((tag) => tag.name.toLowerCase() === name.toLowerCase())) {
      handleError({
        response: {
          data: {
            title: "Duplicate tag",
  ...
  const handleNewTagBlur = async (index) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-start overflow-y-auto scroll-hide">
