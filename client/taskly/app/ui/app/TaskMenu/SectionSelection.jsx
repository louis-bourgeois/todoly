...
export default function SectionSelection({
  ...
  const { sections, modifySection, deleteSection, addSection, modifyTask } =
    useUser();
  const [sectionArrowIsClicked, setSectionArrowIsClicked] = useState(false);
  ...
  const handleNewSectionKeyPress = async (e) => {
    ...
    if (e.key === "Enter" && newSection.name.trim() !== "") {
      try {
  ...
  const handleEditingSectionKeyPress = (e) => {
    if (e.key === "Enter") {
  ...
  const handleSelectSection = (section) => {
    ...
    setSectionArrowIsClicked(false);
  ...
  return (
    <TaskMenuSectionContainer othersStyles="rounded-full justify-between items-center h-[17.5%] relative">
      ...
      <svg
        ...
        className={`cursor-pointer ${
          sectionArrowIsClicked ? "rotate-180" : ""
        } transition-transform duration-500`}
        viewBox="0 0 29 29"
        width="62.5"
        height="62.5"
        onClick={(e) => setSectionArrowIsClicked((prev) => !prev)}
      >
        <path
          fill="none"
          stroke="#000"
            ...
            <div key={section.id} className="flex items-center p-2">
              {editingSectionId === section.id ? (
                <input
                  ...
                  onKeyPress={(e) => handleEditingSectionKeyPress(e)}
        ...
        {isEditingNewSection && (
          <div className="p-2">
            <input
              ...
              onKeyPress={handleNewSectionKeyPress}
            />
          </div>
        )}
      </div>
